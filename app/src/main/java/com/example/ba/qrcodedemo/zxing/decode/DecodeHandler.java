...
package com.example.ba.qrcodedemo.zxing.decode;
...
public class DecodeHandler extends Handler {
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        if (rawResult != null) {
            ...
            if (handler != null
                    && rawResult.getBarcodeFormat() == BarcodeFormat.QR_CODE) {
                ...
                int cropWidth = activity.getCropWidth();

                if (len <= (cropWidth) / 4) {//二维码在扫描框中的宽度小于扫描框的1/4，放大镜头
                    Camera camera = activity.getCameraManager().getCamera();
                    Camera.Parameters parameters = camera.getParameters();
                    if (parameters.isZoomSupported()) {//支持放大镜头
                        int maxZoom = parameters.getMaxZoom();
                        int zoom = parameters.getZoom();
                        Log.d("sssss", len + "decode: " + (cropWidth) / 4);
                        if (zoom == 0) {
                            zoom = maxZoom / 2;
                        } else {
                            zoom = zoom + 10;
                    ...
                    scandSucceeded(handler, rawResult, source);
                }
            }
        } else {
            if (handler != null) {
    ...
    private void scandSucceeded(Handler handler, Result rawResult, PlanarYUVLuminanceSource source) {
